...
namespace SunsetHotelSystem.UI.Controllers {
    public class ClienteController : ConfigController {
        ...
        public async Task<ActionResult> SobreNosotros() {
            ...
            try {
                HttpResponseMessage responseWAPI = await webAPI.GetAsync("api/TSH_SobreN_Galeria");
                if (responseWAPI.IsSuccessStatusCode) {
                    respuesta = JsonConvert.DeserializeObject<Respuesta<List<TSH_SobreN_Galeria>>>(responseWAPI.Content.ReadAsStringAsync().Result);
                    listaGaleria = respuesta.valorRetorno;
                }//Fin del if.
            } catch (Exception ex) {
                System.Console.Write(ex.ToString());
            }//Fin del try-catch.

            return View(listaGaleria);
